...
# rename columns
colnames(weather_tab) <- c('rank', 'flare_classification', 'date', 'flare_region', 'start_time', 'maximum_time', 'end_time', 'movie')
...
weather_tab <- weather_tab %>% 
  mutate(start_time = make_datetime(year = strtoi(str_sub(start_time, start = 1L, end = 4L), base = 10L), 
                                    ...
                                    sec = 0)) %>% 
  mutate(end_time = make_datetime(year = strtoi(str_sub(end_time, start = 1L, end = 4L), base = 10L), 
                                  ...
                                  sec = 0)) %>% 
  mutate(maximum_time = make_datetime(year = strtoi(str_sub(maximum_time, start = 1L, end = 4L), base = 10L), 
                                      ...
                                      sec = 0)) %>% 
  select(rank, flare_classification, flare_region, start_datetime = start_time, maximum_datetime = maximum_time, end_datetime = end_time, -date)
...
NASA_tab <- NASA_tab %>% 
  ...
  mutate(end_time = paste(paste(substr(start_date, 1, 4), end_date, sep='/'), end_time, sep = ' ')) %>% 
  mutate(CME_time = paste(paste(substr(start_date, 1, 4), CME_date, sep='/'), CME_time, sep = ' ')) %>% 
...
NASA_tab <- NASA_tab %>% 
  ...
  mutate(CME_time = make_datetime(year = strtoi(substr(CME_time, 1, 4), base=10), 
                                  ...
                                  sec = 0)) %>% 
  select(start_datetime = start_time, end_datetime = end_time, start_freq, end_freq, location, NOAA, imp, CME_datetime = CME_time, CME_angle, CME_width, CME_speed)
...
NASA_top50 <- NASA_tab %>% 
  ...
  select(rank, flare_classification=imp, flare_region = NOAA, start_datetime, maximum_datetime=CME_datetime, end_datetime)
...
flare_similarity <- function(E1, E2) {
  ...
  # compares the classifications of flares based on how far away they
  # are from each other numerically. If they have different classification
  # IDs, their similarity is set to zero because exp( -((s1 - s2)^2)) would be
  # nearly zero anyway
  ...
  # region is categorical. Either these flares were in 
  # the same region, or they were not.
  s_region <- function(e1, e2) {
    ifelse(e1 == e2, 1, 0)
  }
  
  # The datetimes are measured by their differences. The smaller the difference,
  # the larger the value that will be returned.
